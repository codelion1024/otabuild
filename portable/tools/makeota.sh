...
makediff() {
  ...
  if [ $BIGVERSION -ge 8 ]; then    # we make block-based OTA for new project since android O
    ...
    if [ $full_bsp_modem = "true" ]; then
      $ANDROID/build/tools/releasetools/ota_from_target_files --block --verbose --worker_threads 8 \
      --package_key $KEY --path $otabuild/linux-x86 --device_specific $ANDROID/device/qcom/common \
    ...
    if [ $full_bsp_modem = "true" ]; then
      $ANDROID/build/tools/releasetools/ota_from_target_files --verbose --worker_threads 8 --package_key $KEY \
      --path $otabuild/linux-x86 --device_specific $ANDROID/device/qcom/common \
      --incremental_from $target_old_file_noradio $target_new_file $diffpack_signed
    else
      $ANDROID/build/tools/releasetools/ota_from_target_files --verbose --worker_threads 8 --package_key $KEY \
      --path $otabuild/linux-x86 --device_specific $ANDROID/device/qcom/common \
